...
if TYPE_CHECKING:
    from neopika.queries import QueryBuilder, Selectable, Table
...
class Term(Node):
    ...
    def wrap_constant(
        ...
        :param val:
            Any value.
        :param wrapper_cls:
            A neopika class which wraps a constant value so it can be handled as a component of the query.
        :return:
    ...
    def isin(
        self, arg: Union[list, tuple, set, frozenset, "Term"]
    ) -> "ContainsCriterion":
...
class ParameterValueWrapper(ValueWrapper):
    def __init__(
        self, parameter: Parameter, value: Any, alias: Optional[str] = None
    ) -> None:
...
class Field(Criterion, JSON):
    def __init__(
        ...
        if isinstance(table, str):
            ...
            from neopika.queries import Table
...
class Case(Criterion):
    ...
    def is_aggregate(self) -> Optional[bool]:
        ...
        return resolve_is_aggregate(
            [
                criterion.is_aggregate or term.is_aggregate
                for criterion, term in self._cases
            ]
...
class Function(Criterion):
    ...
    def get_arg_sql(arg, **kwargs):
        return (
            arg.get_sql(with_alias=False, **kwargs)
