...
from neopika.functions import Coalesce, NullIf, Sum
from neopika.terms import Field
...
class TupleTests(unittest.TestCase):
    ...
    def test_tuple_in_using_neopika_tuples(self):
        q = (
            Query.from_(self.table_abc)
            .select(self.table_abc.foo, self.table_abc.bar)
            .where(
    ...
    def test_tuple_is_aggregate(self):
        ...
        with self.subTest(
            ...
            self.assertEqual(
                None, Tuple(Coalesce("col"), NullIf("col2", 0)).is_aggregate
            )
        ...
        with self.subTest(
            ...
            self.assertEqual(
                True,
                Tuple(Coalesce("col"), Coalesce("col2", 0), Sum("col3")).is_aggregate,
            )
