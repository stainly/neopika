...
from neopika import Criterion, JoinType, Order, Query, Tables
from neopika import analytics as an
from neopika.analytics import Lag, Lead
...
class RankTests(unittest.TestCase):
    ...
    def test_lag_generates_correct_sql(self):
        with self.subTest("without partition by"):
            ...
            self.assertEqual(
                'SELECT LAG("date",1,\'2000-01-01\') OVER(ORDER BY "date") FROM "abc"',
                str(q),
            )
    ...
    def test_lead_generates_correct_sql(self):
        with self.subTest("without partition by"):
            ...
            self.assertEqual(
                'SELECT LEAD("date",1,\'2000-01-01\') OVER(ORDER BY "date") FROM "abc"',
                str(q),
            )
