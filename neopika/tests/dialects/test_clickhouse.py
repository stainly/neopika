...
class ClickHouseUpdateTests(TestCase):
    ...
    def test_update(self):
        q = (
            ClickHouseQuery.update(self.table_abc)
...
class ClickHouseDropQuery(TestCase):
    ...
    def test_drop_database(self):
        ...
        q3 = (
            ClickHouseQuery.drop_database(self.database_xyz)
            .if_exists()
            .on_cluster(self.cluster_name)
        )
    ...
    def test_drop_table(self):
        ...
        q3 = (
            ClickHouseQuery.drop_table(self.table_abc)
            .if_exists()
            .on_cluster(self.cluster_name)
        )
    ...
    def test_drop_dictionary(self):
        ...
        q3 = (
            ClickHouseQuery.drop_dictionary("dict")
            .if_exists()
            .on_cluster(self.cluster_name)
        )
...
class LimitByTests(TestCase):
    ...
    def test_limit_by(self):
        q = (
            ClickHouseQuery.from_(self.table_abc)
            .limit_by(1, "a", self.table_abc.b)
            .select("a", "b", "c")
        )

        self.assertEqual(
            """SELECT "a","b","c" FROM "abc" LIMIT 1 BY ("a","b")""", str(q)
        )

    def test_limit_offset_by(self):
        q = (
            ClickHouseQuery.from_(self.table_abc)
            .limit_offset_by(1, 2, "a", self.table_abc.b)
            .select("a", "b", "c")
        )

        self.assertEqual(
            """SELECT "a","b","c" FROM "abc" LIMIT 1 OFFSET 2 BY ("a","b")""", str(q)
        )

    def test_limit_offset0_by(self):
        q = (
            ClickHouseQuery.from_(self.table_abc)
            .limit_offset_by(1, 0, "a", self.table_abc.b)
            .select("a", "b", "c")
        )
