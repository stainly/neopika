...
from neopika import OracleQuery, Table
from neopika.analytics import Count
...
class SelectTests(unittest.TestCase):
    ...
    def test_limit_query(self):
        ...
        self.assertEqual(
            f"SELECT test FROM table1 FETCH NEXT {limit} ROWS ONLY", str(q)
        )
    ...
    def test_limit_offset_query(self):
        ...
        self.assertEqual(
            f"SELECT test FROM table1 OFFSET {offset} ROWS FETCH NEXT {limit} ROWS ONLY",
            str(q),
        )
